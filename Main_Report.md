...
| Test Case ID              | Test Scenario                                                                                                                 | Request Body (JSON)                                                                                               | Expected Status         | Expected Response                                                                    | Audit Status | Audit Reason                                                                                                                                                                                      |
| :------------------------ | :---------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------- | :---------------------- | :----------------------------------------------------------------------------------- | :----------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
### Danh sách Test Cases cho API Quản lý Sản phẩm
...
| **TC_PROD_29**          | CRUD (Read): Lấy danh sách sản phẩm thành công (GET).                                                                           | Không có                                                                                                                                                            | `200 OK`                          | `[{"id": 1, "name": "...", "price": ...}]`                      | **VALID**    | Mở rộng độ phủ cho thao tác Read (CRUD).                                                                                                                                           |
| **TC_PROD_30**          | CRUD (Read): Tìm kiếm sản phẩm bằng Query Params `?search=Laptop` (GET).                                                        | Không có                                                                                                                                                            | `200 OK`                          | `[{"id": 1, "name": "Laptop Dell", "price": ...}]`              | **VALID**    | Kiểm thử tính năng tìm kiếm của endpoint GET.                                                                                                                                      |
| **TC_PROD_31**          | CRUD (Update): Cập nhật thành công sản phẩm có ID hợp lệ (PUT).                                                                 | `{"name": "Laptop Pro", "price": 150000}`                                                                                                                           | `200 OK`                          | `{"message": "Product updated successfully"}`                   | **VALID**    | Mở rộng độ phủ cho thao tác Update (CRUD).                                                                                                                                         |
| **TC_PROD_32**          | CRUD (Update): Cập nhật sản phẩm với ID không tồn tại (PUT).                                                                    | `{"name": "Laptop Pro", "price": 150000}`                                                                                                                           | `404 Not Found`                   | `{"error": "Product not found"}`                                | **VALID**    | Kiểm thử Update với dữ liệu không tồn tại.                                                                                                                                         |
| **TC_PROD_33**          | CRUD (Delete): Xóa thành công sản phẩm có ID hợp lệ (DELETE).                                                                   | Không có                                                                                                                                                            | `200 OK`                          | `{"message": "Product deleted successfully"}`                   | **VALID**    | Mở rộng độ phủ cho thao tác Delete (CRUD).                                                                                                                                         |
| **TC_PROD_34**          | CRUD (Delete): Xóa sản phẩm với ID không tồn tại (DELETE).                                                                      | Không có                                                                                                                                                            | `404 Not Found`                   | `{"error": "Product not found"}`                                | **VALID**    | Kiểm thử Delete với dữ liệu không tồn tại.                                                                                                                                         |
...
| **TC_PROD_39 (Extend)** | **Security (SQLi on URL Params):** Chèn mã SQL độc hại vào URL khi lấy danh sách (GET `/api/products?search=Laptop' OR 1=1--`). | Không có                                                                                                                                                            | `200 OK` (Trả về rỗng) hoặc `400` | Tùy logic Backend                                               | **VALID**    | Bổ sung SQLi vào phương thức GET, khai thác lỗ hổng qua Parameter thay vì Payload Body.                                                                                            |
| **TC_PROD_40 (Extend)** | **Security (Role Escalation on Update):** Dùng Token User thường để cố gắng Sửa sản phẩm (PUT).                                 | `{"name": "Hacked", "price": 1}`                                                                                                                                    | `403 Forbidden`                   | `{"error": "Admin privileges required"}`                        | **VALID**    | Cần phải test phân quyền (Role Escalation) trên mọi endpoint thay đổi trạng thái (POST, PUT, DELETE), không chỉ riêng POST.                                                        |
...
- **Lý do sửa đổi và đánh giá AI:**
  ...
  - **Về CRUD:** AI đã linh hoạt kết hợp thêm các phương thức `GET`, `PUT`, `DELETE`, và `PATCH` để đảm bảo bao phủ hoàn toàn tính năng quản lý sản phẩm.
...
- **Chi tiết các case (Lấp lỗ hổng của AI):**
  ...
  - **Security - SQLi on URL Params (TC_PROD_39):** AI chỉ tập trung chèn SQLi vào Body của phương thức `POST`. Tôi mở rộng kịch bản này bằng cách khai thác lỗ hổng qua Parameter trên URL (`GET /api/products?search=Laptop' OR 1=1--`).
  - **Security - Role Escalation on Update (TC_PROD_40):** AI đã test phân quyền (Role) trên endpoint `POST`. Tuy nhiên, thực tế đã có nhiều hệ thống quên chặn quyền trên endpoint `PUT` hoặc `DELETE`. Tôi viết thêm case này để ép Backend phải chứng minh rằng mọi thao tác thay đổi trạng thái đều được bảo vệ bởi Admin Token.
